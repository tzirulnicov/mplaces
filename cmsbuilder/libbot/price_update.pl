	...
	Str2Sql($tovars{$key}).'" where ID='.$row->{ID},'price_update',1);
	 ...
	 sql_query('INSERT into dbo_CatWareSimple_old (ID,name,price) values (
		...
		Str2Sql($tovars{$key}).'")','price_update',1);
         ...
         $mail_text_changed.=$row->{name}." - с ".
         $tovars_old{$row->{ID}}{price}."р по ".$tovars{$key}."р<br>";
         $mail_text.='Добавлен новый товар: '.$row->{name}."\n";
...
END:
print "Sending mail...\n";
mail("<h3>Добавлены товары:</h3><p>".($mail_text_added || '(нет добавленных)').
	"<p><h3>Изменения цен</h3><p>".($mail_text_changed || '(нет изменённых)').
	"<p><h3>Удалены</h3><p>".($mail_text_deleted || '(нет удалённых)'));
sub find_key{
 my $fkey=shift;
 for my $key (%tovars){
   next if !$tovars{$key};
...
Cc=>'tz@big-bossa.com',
	ContentType=>'html',
        ...
        Text=>'<b>Excel прайс-лист с mobilecenter.ru обработан.
...
<p><p>
Если у Вас появились вопросы по использованию модуля - пишите на ICQ 306405595 либо на tzirulnicov@mail.ru
