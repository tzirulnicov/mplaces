...
sql_connect();
sql_connect('repair',1);
sql_query('SELECT ID from dbo_CatWareSimple where name="12870"');
#2085-2221
open(FILE,'/www/evoo/evoo.ru/evoo_25092008.sql') or die $!;
$data=join(',',<FILE>);
$data=substr($data,index($data,'`dbo_CatWareSimple`'));
close(FILE);
while($row=$db_shandle->fetchrow_hashref){
   print $row->{ID}."\n";
$idx=index($data,$row->{ID});
$tmp=substr($data,$idx);
$tmp=substr($tmp,index($tmp,"','Tag"));
for($a=0;$a<8;$a++){#19
   $tmp=~s/^.*?'//;
}
$tmp=substr($tmp,0,index($tmp,"'"));
die "Cannot find $idx" if $idx==-1;
#print "|".win2koi(substr($data,$idx-20,1000))."|\n";
print ">".win2koi($tmp)."<\n";
sql_query('UPDATE dbo_CatWareSimple SET name="'.Str2Sql($tmp).'" where ID='.
	$row->{ID},'repair',1);
die "Bad name" if $tmp eq ',' or $tmp!~/[A-Za-z]{2,}/;
}
sql_close(1);
sql_close();
exit;
...
$db_shandle->finish();
sql_query('SELECT period,rules,sopostavil,times from dbo_modImport');
die "Cannot load data from DB" if !(my $modimport=$db_shandle->fetchrow_hashref);
$modimport->{sopostavil}=~s/<br>/\r\n/g;
$modimport->{sopostavil}=~s/яНРНБШИ рЕКЕТНМ //g;
$modimport->{sopostavil}=~s/йНЛОЭЧРЕП ЙЮПЛЮММШИ //g;
$modimport->{sopostavil}=~s/Я ТСМЙЖХЕИ GSM-РЕКЕТНМЮ //g;
...
#exit;
	       if (!$row[3] && !$row[4]){
		  if ($row[0]=~/\d+\. /){
		     $header_root=$row[0];# Категория (Сотовые, коммуникаторы, ...)
		  } else {
		     $header=$row[0];# Производитель (Nokia, HTC, Sony, ...)
		  }
	       } else {
	          $row[0]=~s/яНРНБШИ рЕКЕТНМ //;
		  $row[0]=~s/йНЛОЭЧРЕП ЙЮПЛЮММШИ //;
		  $row[0]=~s/Я ТСМЙЖХЕИ GSM-РЕКЕТНМЮ //;
 ...
 #  print "+$1, ot $2, do $4\n";
   if ($row[3]>=$2 && (!$4 || $row[3]<=$4)){
      ...
      last;
	          ...
	          $tovars{$row[0]}=$row[3] if (($header_root eq '1. яНРНБШЕ РЕКЕТНМШ ' || 
			$header_root eq '8. йНЛЛСМХЙЮРНПШ') && grep{$_=~/^$row[0][ \t]*>/} @sopostavil);
	       }
	       @row=();
	    }
...
#      sql_query('UPDATE dbo_CatWareSimple set insight=1,name="'.
...
sub find_key{
 ...
 for my $key (%tovars){
   next if !$tovars{$key};
   my $dbname=(grep{$_=~/$key[ \t]*>/} @sopostavil)[0];
   $dbname=~s/^[^>]+>[ \t]*//;
   $dbname=~s/[ \t]*$//;
   #print "Processing |$key|, searching in DB '$dbname'...\n";
   return $key if $key eq $fkey;
=head
...
=head
if (!keys %tovars==-1){
   $text='Произошла ошибка, не было обработано ни одного товара';
   dieMail($text);
} else {
   $text='Обработаны товары: '.win2koi(join(', ',keys %tovars));
   dieMail($text,1);
}
=сге
print $tex;
