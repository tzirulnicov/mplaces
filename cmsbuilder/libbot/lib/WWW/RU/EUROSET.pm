...
sub new{
   ...
   my $self={};
   my %arg=@_;
   $self->{sock}=WWW::GET->new('euroset.ru',%arg);
   undef if !$sock->{sock};
   bless $self,$pkg;
   $self;
}
...
# SINCE_URL - "www.3dnews.ru/news/polzovateli_ne_dovolni_sensornoi_klaviaturoi_iphone-267413/", for example.
   my $url_base='catalog/mobile/phones/'.$arg{'GROUP'}.'/';
   ...
   if (!$self->{sock}->get($url,NoEncoding=>1)){
      ...
      $self->{'errmsg'}="Error in WWW::RU::EUROSET->list_item (get 'http://www.euroset.ru$url')";
      return 0;
   }
   my $html=$self->{sock}->{'net_http_content'};
   if ($html!~/$url_base(\w+\/)?\-\/[^"]+/){
      $self->{'errmsg'}="Error in WWW::RU::EUROSET->list_item (incorrect html code)";
      return 0;
   }
   while ($html=~/\/$url_base(\w+\/)?\-\/[^"]+/){
      $html=~s/(\/$url_base(\w+\/)?\-\/[^"]+)//;
...
sub get_topic{
   ...
   my $url=${$self->{links}}[0];
#$url='/catalog/mobile/phones/nokia/-/18202/';
#$url='/catalog/mobile/phones/nokia/-/18299/';
   shift @{$self->{links}};
   $self->{'content_image'}='';
   @{$self->{'content_images'}}=();
   $self->{'content_subj'}='';
   ...
   print "url: $url\n" if $arg{'DEBUG'};
   if (!$self->{sock}->get($url,NoEncoding=>1)){
      $self->{'errmsg'}="Error in WWW::RU::EUROSET->get_topic (get 'http://www.euroset.ru/$url')".$self->{sock}->{errmsg};
      return 0;
   }
   my $html=$self->{sock}->{'net_http_content'};
#   WWW::GET->debug_write($html);
   #goto NEXT_POST if (index($html,'<h2 class="')==-1);# Ignoring parse errors
   if (index($html,'характеристики')==-1){
      $self->{'errmsg'}="Error in WWW::RU::EUROSET->get_topic ('incorrect html code')";
      return 0;
   }
