...
#   $nav=~s/ \/ $tmp//;
   $self->{links_head}->{$url}=~s/Bonsai/koi2win('‚œŒ”¡ ')/e;
   $self->{links_head}->{$url}=~s/Cascade/koi2win('Î¡”À¡ƒ')/e;
   $self->{links_head}->{$url}=~s/Forrest/koi2win('Ï≈”')/e;
   $self->{links_head}->{$url}=~s/Landscape/koi2win('≈ ⁄¡÷')/e;
   $self->{links_head}->{$url}=~s/Modern/koi2win('Ûœ◊“≈Õ≈ŒŒŸ≈')/e;
   $self->{links_head}->{$url}=~s/Traditional/koi2win('Ù“¡ƒ…√…œŒŒŸ ')/e;
   @{$self->{'content_nav'}}=(koi2win('‚œŒ”¡ '),$self->{links_head}->{$url});
   print "Navigation: |".win2koi(join(',',@{$self->{'content_nav'}}))."|\n";
   while($html=~s/class\=coltxt02>([^<]+)<\/TD><TD[^>]+>([^<]+)<\/TD><\/TR>//){
      $tmp=substr($1,0,-1);
      $tmp2=$2;
      $tmp=~s/Height/koi2win('˜Ÿ”œ‘¡ ¬œŒ”¡ ')/e;
      $tmp=~s/Pot size/koi2win('Ú¡⁄Õ≈“ «œ“€œﬁÀ¡')/e;
